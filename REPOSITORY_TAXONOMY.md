...
Provides the canonical application shell for new MinSpec projects.
...
Provides the integration and validation environment where MinSpec contributors confirm that the package-and-recipe installation path works correctly inside a real Symfony host application.

It is also the primary working environment for MinSpec-specific AI/Mate tooling.
...
- Mate-enabled by design for MinSpec development
...
Provides MinSpec-specific development-time AI tooling through Mate/MCP-compatible surfaces.
...
- helps AI assistants inspect and reason about MinSpec projects
...
- future `minspec/*-mate` adapter packages if needed
...
- optional
- vendor-specific
- not required by core MinSpec packages
- must not define MinSpec architecture
...
Provides reusable standards, analysis rules, and validation support for MinSpec repositories.
...
- changes must align with MinSpec AI strategy
...
- must stay optional
- must not define MinSpec doctrine
