...
MinSpec takes security seriously, especially anywhere AI-assisted tooling, recipes, generated files, local development tools, or agent experiments are involved.
...
This policy applies to MinSpec repositories and packages, including:
...
- no writable access to real MinSpec repositories by default
- explicit user control for mutation
- auditable command output

MinSpec should prefer one agent/runtime at a time by default. Advanced users may opt into more complex setups intentionally.
...
MinSpec is currently pre-stable and in foundation-building mode.
